<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-blue-600 hover:text-blue-800" :href="route('companies.index')">Companies</inertia-link>
          ...
          <div class="bg-white rounded shadow overflow-hidden max-w-3xl -mt-4">
            <form @submit.prevent="submit">
              ...
              <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
                ...
                <loading-button :loading="sending" class="btn-indigo ml-auto" type="submit">Update Company</loading-button>
              </div>
            </form>
          </div>
        </div>
  ...
  methods: {
    ...
    destroy() {
      if (confirm('Are you sure you want to delete this Company?')) {
        this.$inertia.delete(this.route('companies.destroy', this.company.id))
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this Company?')) {
        this.$inertia.put(this.route('companies.restore', this.company.id))
      }
    },
  },
}
