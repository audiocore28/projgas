<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Gasoline Stations</h1>
...
<!--         <label class="block text-gray-700">Trashed:</label>
 ...
 -->      </search-filter>
      <inertia-link class="btn-indigo" :href="route('stations.create')">
        <span>Add</span>
        <span class="hidden md:inline">Station</span>
      </inertia-link>
    </div>
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
          ...
          <th class="px-6 pt-6 pb-4">Station Name</th>
         ...
         <tr v-for="station in stations.data" :key="station.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
          ...
          <td class="border-t w-px">
            <inertia-link class="px-4 flex items-center" :href="route('stations.edit', station.id)" tabindex="-1">
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('stations.index', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
