        ...
        <div class="overflow-hidden w-full mb-8 -mt-4">
          <form @submit.prevent="updateMindoroTransaction">
                ...
                <tbody v-for="(transaction, transactionIndex) in updateForm.transactions" :key="transactionIndex" class="bg-white rounded shadow mb-8" :id="`transaction-${transaction.id}`">
                  <tr class="bg-gradient-to-r from-yellow-500 to-blue-600">
                    ...
                    <td @click="toggleRow(transactionIndex)" class="border-t text-blue-100 font-semibold text-sm cursor-pointer" align="center">
                      {{ toPHP(transactionTotalAmt(transaction.id) - getLoadTotalAmt(transaction.id) - transaction.expense - transaction.driver_salary - transaction.helper_salary - transaction.commission) }}
                    </td>
            ...
            <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
              <button v-if="!monthly_mindoro_transaction.deleted_at && $page.auth.user.can.mindoroTransaction.delete" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Transaction</button>
  ...
  methods: {
    ...
    updateMindoroTransaction() {
      this.$inertia.put(this.route('monthly-mindoro-transactions.update', this.monthly_mindoro_transaction.id), this.updateForm, {
        ...
        onFinish: () => {
          ...
          axios.get(`/monthly-mindoro-transactions/${this.monthly_mindoro_transaction.id}/edit`)
            .then(response => {
    ...
    getNetTotal() {
      ...
      for (var i = 0; i < this.updateForm.transactions.length; i++) {
        ...
        // Net
        netTotal += transactionTotalAmt - loadTotalAmt - this.updateForm.transactions[i].expense - this.updateForm.transactions[i].driver_salary - this.updateForm.transactions[i].helper_salary - this.updateForm.transactions[i].commission;
