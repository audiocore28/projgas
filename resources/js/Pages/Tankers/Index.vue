<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">Tankers</h1>
    <div class="mb-6 flex justify-between items-center">
      <search-filter v-model="form.search" class="w-full max-w-md mr-4" @reset="reset" :isVisible="$page.auth.user.can.tanker.restore">
    ...
    <div class="bg-white rounded shadow overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        <tr class="text-left font-bold">
         ...
         <tr v-if="tankers.data.length === 0">
          <td class="border-t px-6 py-4" colspan="4">No tankers found.</td>
        </tr>
    </table>
    </div>
    <pagination :links="tankers.links" />
  </div>
</template>
  ...
  watch: {
    form: {
      handler: throttle(function() {
        ...
        this.$inertia.replace(this.route('tankers.index', Object.keys(query).length ? query : { remember: 'forget' }))
      }, 150),
      deep: true,
    },
  },
