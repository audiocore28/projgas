<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-blue-600 hover:text-blue-800" :href="route('batangas-transactions.index')">Batangas Transaction</inertia-link>
        ...
        <div class="bg-white rounded shadow overflow-hidden max-w-6xl mb-8 -mt-4">
          <form @submit.prevent="updateBatangasTransaction">
                    ...
                    <tr class="bg-gray-200">
                      ...
                      <td>
                        <div class="px-6 py-4 whitespace-nowrap text-left text-sm font-semibold text-gray-500">
                          {{ toPHP(transactionTotalAmt() - totalLoad - 1500 - 650) }}
                        </div>
                      </td>
                    </tr>
                  </tbody>
            ...
            <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
              <button v-if="!batangas_transaction.deleted_at" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Transaction</button>
...
export default {
  ...
  metaInfo: { title: 'Edit Batangas Transaction' },
  layout: Layout,
  components: {
    Icon,
    LoadingButton,
  ...
  data() {
    return {
      ...
      updateForm: {
        date: this.batangas_transaction.date,
        trip_no: this.batangas_transaction.trip_no,
        driver_id: this.batangas_transaction.driver_id,
        helper_id: this.batangas_transaction.helper_id,
        tanker_id: this.batangas_transaction.tanker_id,
        purchases: [],
        details: this.batangas_transaction.details,
        selectedPurchases: this.batangas_transaction.selected_purchases,
      },
    }
  },
  methods: {
    updateBatangasTransaction() {
      this.$inertia.put(this.route('batangas-transactions.update', this.batangas_transaction.id), this.updateForm, {
        onStart: () => this.sending = true,
        onFinish: () => this.sending = false,
      });
    },
    destroy() {
      if (confirm('Are you sure you want to permanently delete this Batangas Transaction and its details?')) {
        this.$inertia.delete(this.route('batangas-transactions.destroy', this.batangas_transaction.id))
      }
    },
    deleteDetailForm(index, productDetailId) {
      if (productDetailId) {
        if (confirm('Are you sure you want to delete this row?')) {
          this.$inertia.delete(this.route('batangas-transaction-details.destroy', productDetailId));
    ...
    onSearchPurchaseChange: throttle(function(term) {
      this.$inertia.get(this.route('batangas-transactions.edit'), {term}, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
      })
    ...
    addNewDetailForm() {
      this.updateForm.details.push({
        id: null,
        batangas_transaction_id: this.batangas_transaction.id,
        date: null,
        dr_no: null,
        client_id: null,
        selectedClient: null,
