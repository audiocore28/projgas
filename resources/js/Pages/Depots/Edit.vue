<template>
  <div>
    <h1 class="mb-8 font-bold text-3xl">
      <inertia-link class="text-blue-600 hover:text-blue-800" :href="route('depots.index')">Depots</inertia-link>
      <span class="text-blue-600 font-medium">/</span> {{ form.name }}
    </h1>

    <trashed-message v-if="depot.deleted_at" class="mb-6" @restore="restore" :canRestore="$page.auth.user.can.depot.restore">
      This depot has been deleted.
    </trashed-message>
          ...
          <div class="bg-white rounded shadow overflow-hidden max-w-3xl -mt-4">
            <form @submit.prevent="submit">
              ...
              <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
                <button v-if="!depot.deleted_at && $page.auth.user.can.depot.delete" class="text-red-600 hover:underline" tabindex="-1" type="button" @click="destroy">Delete Depot</button>
                <loading-button :loading="sending" class="btn-indigo ml-auto" type="submit">Update Depot</loading-button>
              </div>
            </form>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        contact_person: this.depot.contact_person,
        contact_no: this.depot.contact_no,
        email_address: this.depot.email_address,
      },
      // Tabs
      openTab: 1,
  ...
  methods: {
    submit() {
      this.$inertia.put(this.route('depots.update', this.depot.id), this.form, {
    ...
    destroy() {
      if (confirm('Are you sure you want to delete this Depot?')) {
        this.$inertia.delete(this.route('depots.destroy', this.depot.id))
      }
    },
    restore() {
      if (confirm('Are you sure you want to restore this Depot?')) {
        this.$inertia.put(this.route('depots.restore', this.depot.id))
      }
    },
  },
}
